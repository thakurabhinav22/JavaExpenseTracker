...
/**
 * ExpenseTracker - Enhanced expense management application
 * Features: Date-based file storage, monthly budget tracking
 */
public class ExpenseTracker extends JFrame {
    private static final String DATA_FOLDER = "data";
    private static final String WALLET_FILE = "wallet.txt";
    private static final String BUDGET_FILE = "monthly_budget.txt";
    private static final String SECRET_KEY = "666C4DB82D3FE4CA";
    
    private JTable expenseTable;
    private DefaultTableModel tableModel;
    private JTextField amountField, descField;
    private JComboBox<String> categoryCombo;
    private JLabel totalLabel, monthlyLabel, categoryWiseLabel, walletLabel, budgetLabel;
    ...
    private final Color WALLET_COLOR = new Color(59, 130, 246);
    
    public ExpenseTracker() {
        setTitle("Expense Tracker Pro - Indian Edition");
        setSize(1350, 850);
    ...
    private JPanel createHeaderPanel() {
        ...
        JPanel walletPanel = new JPanel(new GridLayout(2, 1, 0, 5));
        walletPanel.setBackground(new Color(255, 255, 255, 30));
        walletPanel.setBorder(new EmptyBorder(10, 20, 10, 20));
        
        JLabel walletTitle = new JLabel("Wallet Balance");
        walletTitle.setFont(new Font("Segoe UI", Font.PLAIN, 12));
        walletTitle.setForeground(new Color(255, 220, 180));
        
        walletLabel = new JLabel(formatCurrency(walletBalance));
        walletLabel.setFont(new Font("Segoe UI", Font.BOLD, 24));
        ...
        addMoneyBtn.setFont(new Font("Segoe UI", Font.BOLD, 12));
        addMoneyBtn.setBackground(Color.WHITE);
        addMoneyBtn.setForeground(PRIMARY);
        addMoneyBtn.setFocusPainted(false);
        addMoneyBtn.setBorderPainted(false);
        addMoneyBtn.setCursor(new Cursor(Cursor.HAND_CURSOR));
        addMoneyBtn.setBorder(new EmptyBorder(10, 20, 10, 20));
        addMoneyBtn.addActionListener(e -> addMoneyToWallet());
        
        JButton setBalanceBtn = new JButton("Set Balance");
        setBalanceBtn.setFont(new Font("Segoe UI", Font.BOLD, 12));
        setBalanceBtn.setBackground(new Color(255, 255, 255, 30));
        setBalanceBtn.setForeground(Color.WHITE);
        setBalanceBtn.setFocusPainted(false);
        setBalanceBtn.setBorderPainted(false);
        setBalanceBtn.setCursor(new Cursor(Cursor.HAND_CURSOR));
        setBalanceBtn.setBorder(new EmptyBorder(10, 20, 10, 20));
        setBalanceBtn.addActionListener(e -> setWalletBalance());
        
        JButton budgetBtn = new JButton("Set Budget");
        budgetBtn.setFont(new Font("Segoe UI", Font.BOLD, 12));
        budgetBtn.setBackground(new Color(255, 255, 255, 30));
        ...
        rightPanel.add(addMoneyBtn);
        rightPanel.add(setBalanceBtn);
    ...
    private JPanel createFilterPanel() {
        ...
        JButton filterBtn = createStyledButton("Apply Filter", WALLET_COLOR, false);
    ...
    private JPanel createStatsPanel() {
        ...
        JPanel walletCard = createStatCard("Wallet Balance", formatCurrency(walletBalance), WALLET_COLOR);
        JLabel wLabel = (JLabel) ((JPanel) walletCard.getComponent(1)).getComponent(0);
        panel.add(walletCard);
        
        JPanel totalCard = createStatCard("Total Expenses", "INR0.00", PRIMARY);
        totalLabel = (JLabel) ((JPanel) totalCard.getComponent(1)).getComponent(0);
        panel.add(totalCard);
        
        JPanel monthlyCard = createStatCard("This Month", "INR0.00", ACCENT);
        monthlyLabel = (JLabel) ((JPanel) monthlyCard.getComponent(1)).getComponent(0);
        panel.add(monthlyCard);
        
        JPanel budgetCard = createStatCard("Monthly Budget", monthlyBudget > 0 ? formatCurrency(monthlyBudget) : "Not Set", new Color(147, 51, 234));
        budgetLabel = (JLabel) ((JPanel) budgetCard.getComponent(1)).getComponent(0);
    ...
    private void addMoneyToWallet() {
        String input = JOptionPane.showInputDialog(this, 
            "Enter amount to add to wallet (INR):", 
            "Add Money", 
            JOptionPane.PLAIN_MESSAGE);
        ...
        if (input != null && !input.trim().isEmpty()) {
            try {
                ...
                if (amount > 0) {
                    ...
                    updateWalletDisplay();
                    JOptionPane.showMessageDialog(this, 
                        String.format("%s added successfully!\nNew Balance: %s", formatCurrency(amount), formatCurrency(walletBalance)),
                        "Success", JOptionPane.INFORMATION_MESSAGE);
                } else {
        ...
        if (input != null && !input.trim().isEmpty()) {
            try {
                ...
                if (amount >= 0) {
                    ...
                    updateWalletDisplay();
                    JOptionPane.showMessageDialog(this, 
                        String.format("Wallet balance set to %s", formatCurrency(walletBalance)),
                        "Success", JOptionPane.INFORMATION_MESSAGE);
                } else {
    ...
    private void updateWalletDisplay() {
        walletLabel.setText(formatCurrency(walletBalance));
        
        if (walletBalance < 0) {
            walletLabel.setForeground(DANGER);
        } else if (walletBalance < 1000) {
            walletLabel.setForeground(PRIMARY);
        } else {
            walletLabel.setForeground(Color.WHITE);
        }
    }
        ...
        try {
            ...
            if (amount > walletBalance) {
                int confirm = JOptionPane.showConfirmDialog(this,
                    String.format("Expense (INR%,.2f) exceeds wallet balance (INR%,.2f).\nWallet will go negative. Continue?", 
            ...
            if (monthlyBudget > 0 && monthlyTotal > monthlyBudget) {
                JOptionPane.showMessageDialog(this, 
                    String.format("⚠ Budget Alert!\nMonthly expenses (INR%,.2f) exceeded budget (INR%,.2f)\nRemaining Balance: INR%,.2f", 
                    monthlyTotal, monthlyBudget, walletBalance), 
                    "Budget Exceeded", JOptionPane.WARNING_MESSAGE);
            } else {
                JOptionPane.showMessageDialog(this, 
                    String.format("Expense added!\nAmount: INR%,.2f\nRemaining Balance: INR%,.2f", amount, walletBalance), 
                    "Success", JOptionPane.INFORMATION_MESSAGE);
            }
    ...
    private void deleteExpense() {
        ...
        if (row >= 0) {
            int confirm = JOptionPane.showConfirmDialog(this, 
                "Delete this expense? Amount will be added back to wallet.", 
                "Confirm", JOptionPane.YES_NO_OPTION);
            if (confirm == JOptionPane.YES_OPTION) {
                String amtStr = tableModel.getValueAt(row, 1).toString().replace(",", "");
                ...
                JOptionPane.showMessageDialog(this, 
                    String.format("Expense deleted!\nINR%,.2f added back to wallet\nNew Balance: INR%,.2f", 
                    amount, walletBalance),
                    "Success", JOptionPane.INFORMATION_MESSAGE);
            }
        } else {
    ...
    private void clearAllExpenses() {
        if (tableModel.getRowCount() > 0) {
            int confirm = JOptionPane.showConfirmDialog(this, 
                "Delete all expenses? All amounts will be added back to wallet.", 
            ...
            if (confirm == JOptionPane.YES_OPTION) {
                ...
                JOptionPane.showMessageDialog(this, 
                    String.format("All expenses cleared!\nINR%,.2f added back to wallet\nNew Balance: INR%,.2f", 
    ...
    private void saveAllExpenses() {
        ...
        for (int i = 0; i < tableModel.getRowCount(); i++) {
            ...
            if (!expensesByDate.containsKey(dateOnly)) {
                expensesByDate.put(dateOnly, new ArrayList<>());
            }
            expensesByDate.get(dateOnly).add(new String[]{dateTime, amount, category, desc});
        }
    ...
    private void saveWalletBalance() {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(WALLET_FILE))) {
            String encrypted = encrypt(String.valueOf(walletBalance));
            writer.write(encrypted);
        } catch (Exception ex) {
            showError("Error saving wallet balance: " + ex.getMessage());
        }
    }
    ...
    private void loadWalletBalance() {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(WALLET_FILE))) {
            ...
            if (line != null) {
                String decrypted = decrypt(line);
                walletBalance = Double.parseDouble(decrypted);
            }
        } catch (Exception ex) {
            showError("Error loading wallet balance: " + ex.getMessage());
            walletBalance = 0;
        }
    }
    ...
    private void saveMonthlyBudget() {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(BUDGET_FILE))) {
            String encrypted = encrypt(String.valueOf(monthlyBudget));
            writer.write(encrypted);
        } catch (Exception ex) {
    ...
    private void loadMonthlyBudget() {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(BUDGET_FILE))) {
            ...
            if (line != null) {
                String decrypted = decrypt(line);
                monthlyBudget = Double.parseDouble(decrypted);
            }
        } catch (Exception ex) {
            monthlyBudget = 0;
        }
    ...
    private String decrypt(String encryptedData) {
        try {
            ...
            showError("Decryption error: " + ex.getMessage());
